...
from ten_player_seer_position_experiment import (
    CSV_FIELDNAMES,
    get_seer_position_configs,
    run_seer_position_experiment,
)


SEEDS = [42, 43, 44, 45, 46]
RESULTS_DIR = Path("results")
RAW_CSV_PATH = RESULTS_DIR / "ten_player_seer_position_multi_seed_raw.csv"
SUMMARY_MARKDOWN_PATH = (
    RESULTS_DIR / "ten_player_seer_position_multi_seed_summary.md"
)
...
def run_seer_position_multi_seed(
    seeds=None,
    num_games=NUM_GAMES,
    configs=None,
):
    ...
    for seed in seeds:
        seed_rows = run_seer_position_experiment(
            num_games=num_games,
            seed=seed,
            configs=configs,
        )
...
def print_multi_seed_summary(summary_rows):
    print("Ten-player seer position multi-seed summary")
    print("-------------------------------------------")
    ...
    for row in summary_rows:
        print(
            ...
            f"First check wolf: "
            f"{row['first_check_found_wolf_rate_mean']:.2f}% | "
            f"Edge check rate: {row['edge_check_rate_mean']:.2f}%"
        )


if __name__ == "__main__":
    raw_result_rows = run_seer_position_multi_seed()
